...
wormbase <- Reduce(function(...) full_join(..., by = "geneId"), wormbase) %>%
  select(-eValue) %>%
  rename(hsapiensBlastpGeneId = ensemblGeneId,
         hsapiensBlastpGeneName = ensemblGeneName)
...
geneData <- Reduce(function(...) left_join(..., by = "geneId"), list(wormbase, ensembl, panther)) %>%
  cruft
