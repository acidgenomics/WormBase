...
list <- mclapply(1:nrow(raw_df), function(a) {
    # Human
    hsapiens <- str_extract_all(raw_df[a, 2], "\\bENSG\\d{11}\\b") %>%
        unlist()
    # Mouse
    mmusculus <- str_extract_all(raw_df[a, 2], "\\bENSMUSG\\d{11}\\b") %>%
        unlist()
    # Fruitfly
    dmelanogaster <- str_extract_all(raw_df[a, 2], "\\bFBgn\\d{7}\\b") %>%
        unlist()
    # Zebrafish
    drerio <- str_extract_all(raw_df[a, 2], "\\bENSDARG\\d{11}\\b") %>%
        unlist()
    list(
        hsapiens = hsapiens,
        mmusculus = mmusculus,
