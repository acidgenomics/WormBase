---
title: "metadata function source data"
...
input <- readr::read_tsv("data-raw/rnai_phenotypes.tsv",
                         col_names = FALSE)
...
entrezGeneId <- ensembl2wormbase(df)
```
...
geneOntology <- ensembl2wormbase(df)
```
...
df <- cbind(basic,
            ...
            interpro)
df <- df[, !duplicated(colnames(df))]
ensembl <- seqcloudr::cleanCells(df)
...
```{r cleanup}
id <- data.frame(stringr::str_split_fixed(id, "\\|", 2))
...
lapply(file.path("data", dataFiles), load, .GlobalEnv)
