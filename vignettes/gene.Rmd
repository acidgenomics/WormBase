...
function returns a [tibble][] `data.frame` that can be manipulated using the
[dplyr][] package or the `base::merge()` function. We recommend using
...
Additionally, special `identifiers`, `report` and `NULL` declarations are
available. These shortcuts are useful for generating summaries and may be
relevant to experiments involving large gene lists.
...
**All fields** for [*skn-1*][] (bZIP transcription factor), using the `NULL`
delcaration:
