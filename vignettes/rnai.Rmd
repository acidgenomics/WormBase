...
[ORFeome][] v1.1 RNAi libraries, as well as the cherrypick libraries designed by
the [Ruvkun Lab][].
...
Plate position should be entered in the same format as the [WORFDB][]. The
function is flexible, and also supports entries in the historical format
(e.g. `GHR-11010@G06`), or without a separator:
...
`rnai()` supports clone mapping for cherrypick libraries from the [Ruvkun
Lab][], including the G protein coupled receptor (GPCR), kinase, nuclear hormone
receptor, and transcription factor libraries. These identifiers require the
following prefixes:
...
Here's how you query well E01 ([*daf-16*][]) from the first plate of the
transcription factor cherrypick library:
