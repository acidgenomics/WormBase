...
```{r convert orfeome.xlsx}
orfeomeRaw <- "data-raw/orfeome.xlsx"
if (!file.exists(orfeomeRaw)) {
  download.file("http://dharmacon.gelifesciences.com/uploadedFiles/Resources/cernai-feeding-library.xlsx",
                orfeomeRaw)
}
df <- read_excel(orfeomeRaw, sheet = 2)
...
```{r save orfeome}
orfeome <- bindOrfMetadata(orfeomeConverted)
...
```{r convert ahringer.xlsx}
ahringerRaw <- "data-raw/ahringer.xlsx"
if (!file.exists(ahringerRaw)) {
...
for (i in 1:length(chromosomes)) {
  ...
  df <- suppressWarnings(read_excel(ahringerRaw, sheet = sheet))
  colnames(df) <- camel(colnames(df))
...
ahringerConverted <- df
```

```{r save ahringer}
ahringer <- bindOrfMetadata(ahringerConverted)
